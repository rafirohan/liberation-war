...
function announceCount(items){
  ...
  if(liveRegion) liveRegion.textContent = `${visible} item${visible === 1 ? '' : 's'} shown`;
}
...
function initLightbox(){
  ...
  function open(i){
    ...
    lbImg.src = src.src;
    lbImg.alt = src.alt;
    lbCap.textContent = src.closest('.gallery-item').querySelector('.cap')?.textContent || src.alt;
...
function initInterviewPlayers(){
  document.querySelectorAll('.play-btn').forEach(btn => {
    ...
    btn.addEventListener('click', () => {
      ...
      btn.setAttribute('aria-label', playing ? 'Pause interview preview' : 'Play interview preview');
      if(playing){
        let start = performance.now() - (parseFloat(bar.dataset.progress || '0') * totalSeconds * 1000);
        const loop = (now) => {
          ...
          if(pct < 1 && playing){ raf = requestAnimationFrame(loop); }
          else if(pct >= 1){ playing = false; btn.innerHTML = playIcon(); durationEl.textContent = formatTime(totalSeconds); bar.dataset.progress = 0; bar.style.width = '0%'; }
        };
...
function pauseIcon(){ return '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M6 5h4v14H6zM14 5h4v14h-4z"/></svg>'; }
...
function initContactForm(){
  ...
  form.addEventListener('submit', (e) => {
    ...
    note.textContent = 'Thank you. Your message has been recorded for the archive team — this demo form does not send data anywhere yet, so connect it to your server or a form service before going live.';
